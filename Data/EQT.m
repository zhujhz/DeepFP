function [iter_results,time] = EQT(max_iter,sigma,M,N,L,Q,chn,w,Pmax,V,Set_zero)
...
for iter = 1:max_iter
    ...
    for l = 1:L
        for q = 1:Q
            ...
            for i = 1:L
                for j = 1:Q
                    ex_X(:,q,l,j,i) = chn(:,:,(l-1)*Q+q,i)*ex_V(:,j,i); 
    ...
    for l = 1:L
        for q = 1:Q
            ...
            if Set_zero
                V(:,q,l) = V(:,q,l)+1/lambda(l)*(w(q,l)*(4+t(q,l))*S(:,l,q,l)-D(:,:,l)*V(:,q,l));
            else
