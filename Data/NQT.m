function [iter_results,time] = NQT(max_iter,sigma,M,N,L,Q,chn,w,Pmax,V,Set_zero)
...
for iter = 1:max_iter
    ...
    for l = 1:L
        for q = 1:Q
            if Set_zero
                V(:,q,l) = V(:,q,l)+1/lambda(l)*(w(q,l)*(100000+t(q,l))*S(:,l,q,l)-D(:,:,l)*V(:,q,l));
            else
