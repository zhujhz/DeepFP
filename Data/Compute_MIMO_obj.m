...
for l = 1:L
    for q = 1:Q
        ...
        for i = 1:L
            for j = 1:Q
                noise = noise+chn(:,:,(l-1)*Q+q,i)*V(:,j,i)*V(:,j,i)'*chn(:,:,(l-1)*Q+q,i)';
            end
        end
        noise = noise-chn(:,:,(l-1)*Q+q,l)*V(:,q,l)*V(:,q,l)'*chn(:,:,(l-1)*Q+q,l)';
        SINR(q,l) = V(:,q,l)'*chn(:,:,(l-1)*Q+q,l)'/(noise)*chn(:,:,(l-1)*Q+q,l)*V(:,q,l);
    end
end
SINR = real(SINR);  
